...
func RegisterService(svc Service, mux *http.ServeMux, pathPrefix string) {
	{
		path := fmt.Sprintf("%s%s", pathPrefix, "/v1/api/tickle")
		l := slog.Default().With(slog.String("method", "GET"), slog.String("path", path), slog.String("function", "ListV1APITickle"))

		mux.HandleFunc(fmt.Sprintf("GET %s", path), func(w http.ResponseWriter, r *http.Request) {
			ctx := r.Context()
			l.DebugContext(ctx, "called")

			res, err := svc.ListV1APITickle(ctx)
			if err != nil {

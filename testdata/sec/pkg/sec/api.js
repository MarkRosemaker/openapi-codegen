 ...
 *   const result = await window.API.listAPIXbrlCompanyfactsCikCikJSON(cik);
 *   const result = await window.API.listFilesCompanyTickersJSON();
 *
 * Environment:
...
// reportApiError broadcasts a failed API call as a "sec:ws-message"
...
function reportApiError(operationId, status, message) {
    window.dispatchEvent(new CustomEvent("sec:ws-message", {
        detail: {
            level: "error",
            persistent: true,
...
window.API = {
    ...
    listAPIXbrlCompanyfactsCikCikJSON: (cik) => apiFetch(`/api/xbrl/companyfacts/CIK${cik}.json`, {
        operationId: "ListAPIXbrlCompanyfactsCikCikJSON",
    }),
    // GET /files/company_tickers.json
    listFilesCompanyTickersJSON: () => apiFetch("/files/company_tickers.json", {
        operationId: "ListFilesCompanyTickersJSON",
    }),
};

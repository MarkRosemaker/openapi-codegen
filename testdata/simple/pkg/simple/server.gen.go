...
func RegisterService(svc Service, mux *http.ServeMux, pathPrefix string) {
	{
		...
		l := slog.Default().With(slog.String("method", "GET"), slog.String("path", path), slog.String("function", "ListItems"))
		...
		mux.HandleFunc(fmt.Sprintf("GET %s", path), func(w http.ResponseWriter, r *http.Request) {
			...
			res, err := svc.ListItems(ctx, &params)
			if err != nil {
...

	{
		...
		l := slog.Default().With(slog.String("method", "GET"), slog.String("path", path), slog.String("function", "GetItem"))

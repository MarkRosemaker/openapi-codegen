 ...
 *   const result = await window.API.listItems();
 *   const result = await window.API.getItem(id);
 *
 * Environment:
...
const API_BASE = "https://api.example.com/v1/items";
const TIMEOUT = 4000;
...
// reportApiError broadcasts a failed API call as a "simple:ws-message"
...
function reportApiError(operationId, status, message) {
    window.dispatchEvent(new CustomEvent("simple:ws-message", {
        detail: {
            level: "error",
            persistent: true,

...
type Service interface {
	ListAPIStatesAll(ctx context.Context) (*ListAPIStatesAllOk, error)
}
...
func RegisterService(svc Service, mux *http.ServeMux, pathPrefix string) {
	{
		...
		l := slog.Default().With(slog.String("method", "GET"), slog.String("path", path), slog.String("function", "ListAPIStatesAll"))

		mux.HandleFunc(fmt.Sprintf("GET %s", path), func(w http.ResponseWriter, r *http.Request) {
			ctx := r.Context()
			l.DebugContext(ctx, "called")

			res, err := svc.ListAPIStatesAll(ctx)
			if err != nil {

 ...
 *   const result = await window.API.getV1PageByPageID(pageID);
 *   const result = await window.API.listV1BlockChildren(blockID);
 *
 * Environment:
...
// reportApiError broadcasts a failed API call as a "notion:ws-message"
...
function reportApiError(operationId, status, message) {
    window.dispatchEvent(new CustomEvent("notion:ws-message", {
        detail: {
            level: "error",
            persistent: true,
...
window.API = {
    // GET /pages/{pageId}
    getV1PageByPageID: (pageID) => apiFetch(`/pages/${pageID}`, {
        operationId: "GetV1PageByPageID",
    }),
    // GET /blocks/{blockId}/children
    listV1BlockChildren: (blockID, params = {}) => {
        const q = new URLSearchParams();
        if (params.pageSize != null) q.set("page_size", String(params.pageSize));
        return apiFetch(`/blocks/${blockID}/children?${q}`, {
            operationId: "ListV1BlockChildren",
        });
    },
};

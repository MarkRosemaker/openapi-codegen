 ...
 *   const result = await window.API.getApiapiByApiid(apiID);
 *
 * Environment:
...
const API_BASE = "https://www.freepublicapis.com";
const TIMEOUT = 4000;
...
// reportApiError broadcasts a failed API call as a "freepublicapis:ws-message"
...
function reportApiError(operationId, status, message) {
    window.dispatchEvent(new CustomEvent("freepublicapis:ws-message", {
        detail: {
            level: "error",
            persistent: true,
...
window.API = {
    // GET /api/apis/{apiId}
    getApiapiByApiid: (apiID) => apiFetch(`/api/apis/${apiID}`, {
        operationId: "GetApiapiByApiid",
    }),
};
